...
function Footer() { return <footer><div><a className="footer-name" href={href()}>Diti Chhaproo</a><span>Thoughtfully considered. Always in progress.</span><a className="footer-resume" href={href('resume')}>Résumé</a></div><div className="socials"><a href={socials.linkedin} target="_blank" rel="noreferrer">LinkedIn</a><a href={socials.github} target="_blank" rel="noreferrer">GitHub</a><a href={socials.email}>diti@illinois.edu</a></div><p><span>© {new Date().getFullYear()} · Champaign, Illinois</span><span className="colophon">An ongoing edition · Written & built by Diti</span></p></footer>; }
...
function CaseStudy({project:p}) {
  ...
  const art=p.artwork||p.slug;
  return <article className="case-study"><section className="case-hero"><a className="text-link" href={href(p.track)}>Vol. {tracks[p.track].volume} / {tracks[p.track].name}</a><p className="eyebrow">{p.category}</p><h1>{p.title}<em>{p.subtitle}</em></h1><p className="case-lead">{p.lead} <strong>{p.emphasis}</strong></p><img className="case-hero-image" src={`/assets/portfolio/${art}.svg`} alt={`Illustrative overview of ${p.title}`}/></section><nav className="jump-nav" aria-label="Case study sections">{['Problem','Approach','Artifacts','Takeaways'].map(s=><button className={`ribbon ribbon-${({Problem:"rose",Approach:"peach",Artifacts:"gold",Takeaways:"sage"})[s]} ${activeSection===s.toLowerCase()?"is-active":""}`} aria-current={activeSection===s.toLowerCase()?"location":undefined} key={s} onClick={()=>{setActiveSection(s.toLowerCase());const el=document.getElementById(s.toLowerCase());el?.scrollIntoView({behavior:window.matchMedia('(prefers-reduced-motion: reduce)').matches?'instant':'smooth'});el?.focus({preventScroll:true});}}><span>{s}</span><i aria-hidden="true"/></button>)}</nav><dl className="metadata">{[['Role',p.role],['Timeline',p.timeline],['Tools',p.tools],['Context',p.context]].map(([k,v])=><div key={k}><dt>{k}</dt><dd>{v}</dd></div>)}</dl><section id="problem" tabIndex="-1" className="problem-callout"><p className="eyebrow">The problem</p><h2>{p.problem}</h2></section><section id="approach" tabIndex="-1" className="case-section"><h2>The approach</h2><div className="approach-grid">{p.approach.map(([title,body])=><div key={title}><h3>{title}</h3><p>{body}</p></div>)}</div></section><section id="artifacts" tabIndex="-1" className="case-section"><div className="section-heading"><h2>Working artifacts</h2><span>Preview collection</span></div><div className="artifact-grid">{p.artifacts.map((a,i)=><figure key={a}><img src={`/assets/portfolio/${art}.svg`} alt={`Placeholder schematic for ${a}`} className={i?'detail-crop':''}/><figcaption><strong>{a}</strong><span>Illustrative placeholder · original artifact forthcoming</span></figcaption></figure>)}</div></section><section id="takeaways" tabIndex="-1" className="case-section takeaways"><h2>What I’m taking forward</h2><ol>{p.takeaways.map(t=><li key={t}>{t}</li>)}</ol></section><div className="case-end"><a href={href(p.track)}>All {p.track==='engineer'?'engineering':'product'} work</a><a href={href('contact')}>Let’s talk about the work</a></div></article>; }
function About() {
...
function Contact() { return <section className="simple-page"><p className="eyebrow">Start a conversation</p><h1>Good work starts<br/><em>with a connection.</em></h1><p>For engineering, product, research, or a thoughtful exchange of ideas.</p><a className="contact-email" href={socials.email}>diti@illinois.edu</a><div className="inline-links"><a href={socials.linkedin} target="_blank" rel="noreferrer">LinkedIn</a><a href={socials.github} target="_blank" rel="noreferrer">GitHub</a></div><p className="eyebrow">Champaign, Illinois</p></section>; }
function Resume() { return <section className="simple-page"><p className="eyebrow">Experience, on paper</p><h1>The <em>résumé.</em></h1><p>An updated PDF will be added here soon.</p><p>Systems Engineering and Design · UIUC Grainger<br/>Spring 2028 · CS minor · Quantum SFO</p><a className="text-link" href="mailto:diti@illinois.edu?subject=Resume%20request">Request my resume</a><div className="inline-links"><a href={href('engineer')}>Explore engineering work</a><a href={href('builder')}>Explore product work</a></div></section>; }
